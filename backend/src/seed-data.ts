...
export function generateSteps(title: string, count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (step.length < 120) {
      ...
      const pad =
        " Carefully follow the timings and watch for visual cues so you can reproduce this result.";
      while (step.length + pad.length < 120) step += pad;
      if (step.length > 150) step = step.slice(0, getRandomInt(120, 150));
...
export function loadImageFiles(): string[] {
